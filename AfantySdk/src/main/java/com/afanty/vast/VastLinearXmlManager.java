...
class VastLinearXmlManager {
    private static final String TAG = "Ad.VastLinearXmlManager";
    ...
    List<VastFractionalProgressTracker> getFractionalProgressTrackers() {
        ...
        addQuartileTrackerWithFraction(percentTrackers, getVideoTrackersByAttribute(FIRST_QUARTILE), FIRST_QUARTER_MARKER, FIRST_QUARTILE);
        addQuartileTrackerWithFraction(percentTrackers, getVideoTrackersByAttribute(MIDPOINT), MID_POINT_MARKER, MIDPOINT);
        addQuartileTrackerWithFraction(percentTrackers, getVideoTrackersByAttribute(THIRD_QUARTILE), THIRD_QUARTER_MARKER, THIRD_QUARTILE);
    ...
    List<VastAbsoluteProgressTracker> getAbsoluteProgressTrackers() {
        ...
        for (String url : startTrackers) {
            trackers.add(new VastAbsoluteProgressTracker(url, START_TRACKER_THRESHOLD, START));
        }
                ...
                if (Strings.isAbsoluteTracker(offsetString)) {
                    ...
                    try {
                        ...
                        if (trackingMilliseconds != null && trackingMilliseconds >= 0) {
                            trackers.add(new VastAbsoluteProgressTracker(trackingUrl, trackingMilliseconds, PROGRESS));
                        }
                    } catch (NumberFormatException e) {
                    }
                }
                ...
                if (creativeNodeValue != null) {
                    trackers.add(
                            new VastAbsoluteProgressTracker(creativeNodeValue,
                                    CREATIVE_VIEW_TRACKER_THRESHOLD, CREATIVE_VIEW));
                }
            }
        }
    ...
    List<VastTracker> getPauseTrackers() {
        ...
        for (String tracker : trackers) {
            vastRepeatableTrackers.add(new VastTracker(tracker, true, PAUSE));
        }
    ...
    List<VastTracker> getResumeTrackers() {
        ...
        List<VastTracker> vastRepeatableTrackers = new ArrayList<VastTracker>();
        for (String tracker : trackers) {
            vastRepeatableTrackers.add(new VastTracker(tracker, true, RESUME));
        }
        return vastRepeatableTrackers;
    }

    @NonNull
    List<VastTracker> getMuteTrackers() {
        List<String> trackers = getVideoTrackersByAttribute(MUTE);
        List<VastTracker> vastRepeatableTrackers = new ArrayList<VastTracker>();
        for (String tracker : trackers) {
            vastRepeatableTrackers.add(new VastTracker(tracker, true, MUTE));
        }
        return vastRepeatableTrackers;
    }

    List<VastTracker> getUnMuteTrackers() {
        List<String> trackers = getVideoTrackersByAttribute(UNMUTE);
        List<VastTracker> vastRepeatableTrackers = new ArrayList<VastTracker>();
        for (String tracker : trackers) {
            vastRepeatableTrackers.add(new VastTracker(tracker, true, UNMUTE));
        }
        return vastRepeatableTrackers;
    }
    ...
    @NonNull
    String getDuration() {
        final Node DurationNode = XmlUtils.getFirstMatchingChildNode(mLinearNode, DURATION);
        final String duration = XmlUtils.getNodeValue(DurationNode);
        if (duration == null) {
            return null;
        }
    ...
    private List<VastTracker> getVideoTrackersByAttributeAsVastTrackers(
        ...
        for (String tracker : trackers) {
            vastTrackers.add(new VastTracker(tracker, attributeValue));
        }
        return vastTrackers;
    }
    ...
    private void addQuartileTrackerWithFraction(
            ...
            @NonNull final List<String> urls, float fraction, String attribute) {
        Preconditions.checkNotNull(trackers, "trackers cannot be null");
        Preconditions.checkNotNull(urls, "urls cannot be null");
        for (String url : urls) {
            trackers.add(new VastFractionalProgressTracker(url, fraction, attribute));
        }
    }
}
